...
void Server::stateInfo(SOCKET s, int no,string info)
{
	...
	switch (no)
	{
		case 0:qs = "Server socket create ERROR !\n";  break; 						//表示服务器监听socket创建失败
		case 1:qs = "Server socket create OK!\n"; break; 							//表示服务器监听socket创建成功
		case 2:qs = "Server socket bind ERROR!\n"; break;							//表示服务器监听socket绑定地址端口失败
		case 3:qs = "Server socket bind OK!\n"; break;								//表示服务器监听socket绑定地址端口成功
		case 4:qs= "Server socket listen ERROR!\n"; break;					//表示服务器监听socket开始监听
		case 5:qs="Server socket listen OK!\n"; break;						//表示服务器监听socket监听失败
		case 6:qs="Server accept connection request ERROR!\n"; break;		//表示接收连接请求失败
		case 7:qs="Server accept connection request OK!\n"; break;			//表示接收连接请求成功
		case 8:qs="ioctlsocket() for new session failed with ERROR!\n";break;//ioctlsocket() for new session failed with ERROR!
		case 9:qs="ioctlsocket() failed with ERROR!\n"; break;				//ioctlsocket() failed with ERROR!
		case 10:qs="ioctlsocket() for server socket OK!Waiting for client connection and data\n"; break;//ioctlsocket() for server socket OK!Waiting for client connection and data
		case 11:qs="\t" + QString::fromStdString(comb)+ " New thread Create OK!\n"; break;		//创建进程成功
		case 12:qs="\t" + QString::fromStdString(comb) + " New thread Create ERROR!\n"; break;				//创建进程失败
		case 13:qs="select() failed with ERROR!\n"; break;					//表示select失败
		case 14:qs="\t" +QString::fromStdString(comb)+ " Server receive request ERROR !\n"; break;		//表示接收数据失败
		case 15:qs="\t" + QString::fromStdString(comb) + " Server receive request OK !\n"; break;			//表示接收数据成功	
		case 16:qs="\t" + QString::fromStdString(comb) + " Server send respond ERROR !\n"; break;			//表示发送数据失败
		case 17:qs="\t" + QString::fromStdString(comb) + " Server send respond OK !\n"; break;				//表示发送数据成功
		case 18:qs="\t" + QString::fromStdString(comb) + " Request Timeout\n"; break;						//表示连接超时
		case 19:qs="\t" + QString::fromStdString(comb) + " Request Header:" + QString::fromStdString(info) +"\n"; break;				//表示连接超时
		case 20:qs = "\t" + QString::fromStdString(comb) + " Can't find the file:" + QString::fromStdString(info) + "\n"; break;
	}
	QString temp =  this->GUI->label->text();
	temp = temp + qs;
	this->GUI->label->setText(temp);
	consoleLock.unlock();
}
...
void Server::setGUI(HTTPServerGUI *GUI)
{
	this->GUI = GUI;
}

bool Server::opened(void)
{
	return this->open;
}