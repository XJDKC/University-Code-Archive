...
REFRESH:
	...
	PUSH DI
	CALL CAL_PROFIT
	MOV AX,DS:[SI+18]					;获得一号商店该商品利润率
	ADD AX,DS:[DI+18]					;获得二号商店该商品利润率
	SAR AX,1
	MOV [SI+18],AX
	ADD SI,20
;	ADD BP,20
	CMP SI,OFFSET GA1+20*N
	JB LOOP2
...
CAL_PROFIT PROC						;计算商品利润率并保存到对应位置 参数--商品信息存放位置
	PUSH BP							;栈:商品(2) 子程序返回地址(2) BP(2)
	MOV BP,SP						;栈:商品(2) 子程序返回地址(2) BP(2)
	XCHG SI,[BP+4]					;栈:SI(2) 子程序返回地址(2) BP(2)
	...
	MOV EDX,0						;高位补0
	MOVSX EAX,WORD PTR [SI+10+2*1]	;计算总收入 EAX=售价
	MOVSX ECX,WORD PTR [SI+10+2*3]	;ECX=售货量
	IMUL EAX,ECX					;EAX=总收入
	MOVSX EBX,WORD PTR [SI+10]		;计算成本 EBX=进价
	MOVSX ECX,WORD PTR [SI+10+2*2]	;ECX=进货量
	IMUL EBX,ECX					;EBX=总成本
	...
	MOV SI,[BP+4]					;栈:SI(2) 子程序返回地址(2) BP(2)
	POP BP							;栈:SI(2) 子程序返回地址(2)
	RET 2							;跳转到栈顶指向的IP 忽略2个字节
CAL_PROFIT ENDP

FIND_PROD1 PROC						;参数商店信息偏移量 待查找商品名偏移量 返回值存在AX
	PUSH BP							;栈:商店(2) 商品名(2) 子程序返回地址(2) BP(2)
	MOV BP,SP						;栈:商店(2) 商品名(2) 子程序返回地址(2) BP(2)
	PUSH CX							;栈:商店(2) 商品名(2) 子程序返回地址(2) BP(2)  CX(2)
	XCHG SI,[BP+6]					;存放商店信息偏移量		栈:SI(2) 商品名(2) 子程序返回地址(2) BP(2)  CX(2)
	...
	MOV CX,N
LOOPP6:								;循环找到DI(输入的商品名称)对应的商品
	PUSH SI							;传参--将当前商品名称入栈
	PUSH DI							;传参--将查找商品名称入栈
	CALL JUDGE1						;调用子程序比较名称是否相等
	CMP AX,0						;比较返回结果是否相等
	JNZ FIND1						;找到跳出循环
	ADD DI,20						;否则跳转到下一个商品位置 (一个商品占20个字节)
	LOOP LOOPP6						;如果还有商品就继续循环

	MOV AX,0						;没找到则返回0
	JMP FIND_PROD_RET1				;跳转到返回处
FIND1:MOV AX,DI						;将当前店铺与输入商品名称对应的商品信息偏移量返回
FIND_PROD_RET1:						;返回子程序
	POP CX							;栈:SI(2) DI(2) 子程序返回地址(2) BP(2)
	MOV DI,[BP+4]					;栈:SI(2) DI(2) 子程序返回地址(2) BP(2)
	MOV SI,[BP+6]					;栈:SI(2) DI(2) 子程序返回地址(2) BP(2)
	POP BP							;栈:SI(2) DI(2) 子程序返回地址(2)
...
JUDGE1 PROC							;参数字符串原串 目的串 
	...
	REPE CMPSB						;开始比较
	JNZ NO1							;如果不相等则返回0
YES1:MOV AX,1						;否则返回1 将返回值放在AX中
	JMP JUDGE_RET1					;跳转到返回处
NO1:	MOV AX,0						;否则返回0
JUDGE_RET1:							
	POP CX							;栈:DI(2) SI(2) 子程序返回地址(2) BP(2)  BX(2)
	POP BX							;栈:DI(2) SI(2) 子程序返回地址(2) BP(2)
